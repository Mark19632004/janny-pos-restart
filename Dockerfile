...
COPY package*.json ./
RUN npm install
# Install backend dependencies and plugin not included in package.json
RUN npm install express cors pdfkit uuid dayjs dotenv @prisma/client prisma @vitejs/plugin-react
...
# Install backend dependencies again (including plugin)
RUN npm install express cors pdfkit uuid dayjs dotenv @prisma/client prisma @vitejs/plugin-react
